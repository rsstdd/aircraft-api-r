...
CREATE DOMAIN aircraft_ref.nonneg_numeric AS NUMERIC
    CHECK (VALUE >= 0);
COMMENT
ON DOMAIN aircraft_ref.nonneg_numeric IS
...
COMMENT
ON DOMAIN aircraft_ref.confidence_score IS
...
COMMENT
ON DOMAIN aircraft_ref.year_value IS
...
COMMENT
ON DOMAIN aircraft_ref.slug_text IS
...
COMMENT
ON DOMAIN aircraft_ref.lookup_code IS
...
CREATE
OR REPLACE FUNCTION aircraft_ref.set_updated_at()
RETURNS trigger
LANGUAGE plpgsql AS $$
BEGIN
  NEW.updated_at
...
$$;
COMMENT
ON FUNCTION aircraft_ref.set_updated_at() IS
...
CREATE
OR REPLACE FUNCTION aircraft_ref.slugify(input text)
RETURNS text
LANGUAGE sql IMMUTABLE AS $$
SELECT trim(both '-' FROM regexp_replace(lower(coalesce(input, '')), '[^a-z0-9]+', '-', 'g'));
$$;
COMMENT
ON FUNCTION aircraft_ref.slugify(text) IS
...
CREATE
OR REPLACE FUNCTION aircraft_ref.normalize_lookup_code(input text)
RETURNS text
LANGUAGE sql IMMUTABLE AS $$
