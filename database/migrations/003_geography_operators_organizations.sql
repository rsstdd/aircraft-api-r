...
COMMENT
ON TABLE aircraft_geo.countries IS
...
COMMENT
ON COLUMN aircraft_geo.countries.continent IS
...
COMMENT
ON COLUMN aircraft_geo.countries.is_active IS
...
COMMENT
ON TABLE aircraft_geo.regions IS
...
COMMENT
ON COLUMN aircraft_geo.regions.region_type IS
...
COMMENT
ON TABLE aircraft_geo.country_regions IS
...
COMMENT
ON TABLE aircraft_org.organizations IS
...
COMMENT
ON COLUMN aircraft_org.organizations.extra_attributes IS
...
COMMENT
ON TABLE aircraft_org.org_relationships IS
...
COMMENT
ON COLUMN aircraft_org.org_relationships.from_org_id IS
    'The originating / subordinate / predecessor organization in the relationship.';
COMMENT
ON COLUMN aircraft_org.org_relationships.to_org_id IS
...
CREATE INDEX idx_countries_continent
    ON aircraft_geo.countries (continent) WHERE is_active; -- active-country continent facet is the hot path
CREATE INDEX idx_countries_inactive
    ON aircraft_geo.countries (code) WHERE NOT is_active;
-- historical states: small, separate scan
...
CREATE UNIQUE INDEX uq_org_icao_mfr
    ON aircraft_org.organizations (icao_mfr_code) WHERE icao_mfr_code IS NOT NULL;

CREATE INDEX idx_org_type
    ON aircraft_org.organizations (org_type_code);
CREATE INDEX idx_org_country
    ON aircraft_org.organizations (country_code) WHERE country_code IS NOT NULL;
...
CREATE UNIQUE INDEX uq_org_rel_active
    ON aircraft_org.org_relationships
        (from_org_id, to_org_id, relationship_type_code) WHERE ended_year IS NULL; -- only one active relationship of each type per pair
...
INSERT INTO aircraft_geo.countries (code, alpha2, name, continent, is_active)
VALUES
    ...
    -- Russian Federation: required for Russian design bureaus (Sukhoi, Mikoyan)
    -- and CIS membership. Distinct from the historical Soviet Union (SUN) below.
    ('RUS', 'RU', 'Russia', 'EUROPE', TRUE),
    ...
    ('DZA', 'DZ', 'Algeria', 'AFRICA', TRUE) ON CONFLICT (code) DO NOTHING;

-- =============================================================================
-- SEED DATA — regions (11 rows)
-- =============================================================================

INSERT INTO aircraft_geo.regions (code, label, description, region_type, sort_order)
VALUES
    ...
    -- Military alliance
    ('NATO', 'NATO',
     'North Atlantic Treaty Organisation member states.', 'MILITARY', 100) ON CONFLICT (code) DO NOTHING;
...
INSERT INTO aircraft_geo.country_regions (country_code, region_code)
VALUES ('USA', 'NORTH_AMERICA'),
       ('CAN', 'NORTH_AMERICA'),
       ('MEX', 'NORTH_AMERICA'),
       ('CUB', 'NORTH_AMERICA') ON CONFLICT DO NOTHING;

-- Geographic: LATIN_AMERICA
INSERT INTO aircraft_geo.country_regions (country_code, region_code)
VALUES ('BRA', 'LATIN_AMERICA'),
       ('ARG', 'LATIN_AMERICA'),
       ('COL', 'LATIN_AMERICA'),
       ('CHL', 'LATIN_AMERICA'),
       ('PER', 'LATIN_AMERICA'),
       ('VEN', 'LATIN_AMERICA'),
       ('URY', 'LATIN_AMERICA') ON CONFLICT DO NOTHING;
...
VALUES ('GBR', 'EUROPE'),
       ...
       ('DDR', 'EUROPE'),
       ('CSK', 'EUROPE'),
       ('YUG', 'EUROPE') ON CONFLICT DO NOTHING;

-- Geographic: MIDDLE_EAST
INSERT INTO aircraft_geo.country_regions (country_code, region_code)
VALUES ('ISR', 'MIDDLE_EAST'),
       ('SAU', 'MIDDLE_EAST'),
       ('ARE', 'MIDDLE_EAST'),
       ('IRN', 'MIDDLE_EAST'),
       ('IRQ', 'MIDDLE_EAST'),
       ...
       ('JOR', 'MIDDLE_EAST'),
       ('KWT', 'MIDDLE_EAST'),
       ('OMN', 'MIDDLE_EAST') ON CONFLICT DO NOTHING;

-- Geographic: ASIA_PACIFIC
INSERT INTO aircraft_geo.country_regions (country_code, region_code)
VALUES ('CHN', 'ASIA_PACIFIC'),
       ('JPN', 'ASIA_PACIFIC'),
       ('KOR', 'ASIA_PACIFIC'),
       ('PRK', 'ASIA_PACIFIC'),
       ('TWN', 'ASIA_PACIFIC'),
       ...
       ('AZE', 'ASIA_PACIFIC'),
       ('AUS', 'ASIA_PACIFIC'),
       ('NZL', 'ASIA_PACIFIC') ON CONFLICT DO NOTHING;
...
INSERT INTO aircraft_geo.country_regions (country_code, region_code, joined_year)
VALUES ('DEU', 'EU', 1958),
       ('FRA', 'EU', 1958),
       ('ITA', 'EU', 1958),
       ('NLD', 'EU', 1958),
       ...
       ('BGR', 'EU', 2007),
       ('ROU', 'EU', 2007),
       ('HRV', 'EU', 2013) ON CONFLICT DO NOTHING;
...
VALUES ('USA', 'NATO', 1949),
       ...
       ('HRV', 'NATO', 2009),
       ('FIN', 'NATO', 2023),
       ('SWE', 'NATO', 2024) ON CONFLICT DO NOTHING;
...
VALUES
    ...
    ('embraer',
     ...
     'São José dos Campos; ERJ, E-Jet, E2 regional jets; '
         'Phenom, Praetor business jets; KC-390 military transport.') ON CONFLICT (slug) DO NOTHING;
