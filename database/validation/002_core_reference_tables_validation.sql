...
--   seeds/002_reference_units.sql
--   seeds/001_lookup_seed_data.sql
...
WITH expected AS (
    SELECT * FROM (VALUES
                       ('aircraft_ref', 'unit_categories',              15),
                       ...
                       ('aircraft_ref', 'equipment_provision_types',     7)
                  ) AS t(schema_name, table_name, expected_count)
),
     actuals AS (
         SELECT
...
-- -----------------------------------------------------------------------------
SELECT 'performance_metric_types' AS tbl, code, canonical_unit_code,
       CASE WHEN mu.code IS NOT NULL OR canonical_unit_code IS NULL
                THEN 'OK' ELSE 'BROKEN FK' END AS status
...
UNION ALL
SELECT 'weight_metric_types', code, canonical_unit_code,
       CASE WHEN mu.code IS NOT NULL OR canonical_unit_code IS NULL
                THEN 'OK' ELSE 'BROKEN FK' END
FROM aircraft_ref.weight_metric_types wmt
         LEFT JOIN aircraft_ref.measurement_units mu ON mu.code = wmt.canonical_unit_code
UNION ALL
SELECT 'dimension_metric_types', code, canonical_unit_code,
       CASE WHEN mu.code IS NOT NULL OR canonical_unit_code IS NULL
                THEN 'OK' ELSE 'BROKEN FK' END
...
-- -----------------------------------------------------------------------------
SELECT pc.code, pc.primary_power_unit,
       CASE WHEN mu.code IS NOT NULL OR pc.primary_power_unit IS NULL
                THEN 'OK' ELSE 'BROKEN FK' END AS status
...
-- -----------------------------------------------------------------------------
SELECT 'aircraft_roles' AS tbl, code FROM aircraft_ref.aircraft_roles
WHERE code !~ '^[A-Z][A-Z0-9_]*$'
UNION ALL
SELECT 'service_statuses', code FROM aircraft_ref.service_statuses
WHERE code !~ '^[A-Z][A-Z0-9_]*$'
UNION ALL
SELECT 'measurement_units', code FROM aircraft_ref.measurement_units
WHERE code !~ '^[A-Z][A-Z0-9_]*$'
UNION ALL
SELECT 'performance_metric_types', code FROM aircraft_ref.performance_metric_types
WHERE code !~ '^[A-Z][A-Z0-9_]*$'
UNION ALL
SELECT 'cost_item_types', code FROM aircraft_ref.cost_item_types
WHERE code !~ '^[A-Z][A-Z0-9_]*$';
...
SELECT
    ...
    'unit_categories','measurement_units'))                     AS unit_tables,
    count(*) FILTER (WHERE table_name IN (
    'aircraft_roles','service_statuses','variant_types'))       AS taxonomy_tables,
    count(*) FILTER (WHERE table_name IN (
    'landing_gear_types','propulsion_categories','fuel_types'))  AS physical_tables,
    count(*) FILTER (WHERE table_name IN (
    'performance_metric_types','weight_metric_types',
    'dimension_metric_types'))                                  AS metric_type_tables,
    count(*)                                                      AS total_lookup_tables
