-- =============================================================================
-- File: database/migrations/902_server_side_json_ingestion.sql
...
BEGIN
    v_run_label := 'planephd_' || to_char(now(), 'YYYYMMDD_HH24MI');
...
BEGIN
    BEGIN
        v_count := aircraft_ingest.load_seed_json(:'seed_json_path');
        RAISE NOTICE '902: Staged % aircraft records.', v_count;
    EXCEPTION WHEN OTHERS THEN
        RAISE NOTICE '902: Skipping load (seed_json_path not set or unreadable): %', SQLERRM;
    END;
    RAISE NOTICE '902: Starting promotion pass...';
END;
$$;
