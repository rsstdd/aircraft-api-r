use crate::{location_info, settings::structs::Settings, AircraftError};
...
impl Settings {
  ...
  ///
  /// Note: The env var `DATABASE_URL` is parsed in
  /// `aircraft_db_schema/src/lib.rs::get_database_url_from_env()`
  /// Warning: Only call this once.
  pub fn init() -> Result<Self, AircraftError> {
  ...
  pub fn get_config_location() -> String {
    env::var("AIRCRAFT_CONFIG_LOCATION").unwrap_or_else(|_| DEFAULT_CONFIG_FILE.to_string())
  }
  ...
  /// variable will be like `aircraft-alpha:8541`. This method removes the port and returns
  /// `aircraft-alpha` instead. It has no effect in production.
  ...
  pub fn save_config_file(data: &str) -> Result<String, AircraftError> {
