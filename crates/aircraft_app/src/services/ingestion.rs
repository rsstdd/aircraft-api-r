...
pub enum PreparedInputError {
    ...
    #[error("record stream ended unexpectedly: {0}")]
    Internal(String),
}
...
impl PreparedInputError {
    ...
    pub fn code(&self) -> &str {
        match self {
            ...
            Self::Internal(_) => "RECORD_STREAM_INTERNAL",
        }
    }
}
...
impl IngestionService {
    ...
    pub async fn import(
        ...
        let (run_id, attempt_id, mut unit_of_work) = match self.store.start_import(&request).await?
        {
            ...
            ImportStart::Ready { run_id, attempt_id, unit_of_work } => {
                (run_id, attempt_id, unit_of_work)
            }
        };

        let result = self
            .consume_records(&request, records, unit_of_work.as_mut(), run_id, attempt_id)
            .await;
    ...
    async fn consume_records(
        ...
        while let Some(record) = records.next_record().await? {
            ...
            if let Some(issue) =
                record.issues.iter().find(|issue| issue.severity == IssueSeverity::Error)
            {
            ...
            match outcome.disposition {
                ...
                RecordDisposition::Skipped => report.skipped_records += 1,
            }
...
mod tests {
    ...
    impl PreparedRecordReader for FakeReader {
        async fn next_record(
            &mut self,
        ) -> Result<Option<PreparedAircraftRecord>, PreparedInputError> {
    ...
    async fn warning_records_commit_and_remain_flagged() {
        ...
        let report = service
            .import(request(1), &mut reader)
            .await
            .expect("warning-only record should commit");
    ...
    async fn commit_failure_persists_failure_audit() {
        let state =
            Arc::new(std::sync::Mutex::new(State { fail_commit: true, ..State::default() }));
        let service = IngestionService::new(Arc::new(FakeStore { state: Arc::clone(&state) }));
        let mut reader = FakeReader { records: [record(IssueSeverity::Warning)].into() };
