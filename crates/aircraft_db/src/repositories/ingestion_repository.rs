use aircraft_app::ingestion::{
    ...
    RecordDisposition, RecordOutcome, RunStatus, StatusFilter,
};
    ...
    async fn successful_report(
        ...
        let row = query(
            "SELECT r.id, a.id AS attempt_id, r.staged_aircraft,
                    r.promoted_aircraft, r.flagged_aircraft, r.skipped_aircraft,
                    r.warning_count
        ...
        let attempt_id = row
            .try_get::<Option<i64>, _>("attempt_id")
            .map_err(database_error)?
            .ok_or_else(|| {
        ...
        Ok(Some(ImportReport {
            ...
            skipped_records: nonnegative(row.try_get("skipped_aircraft").ok()),
...
impl IngestionStore for SqlxIngestionStore {
    ...
    async fn start_import(&self, request: &ImportRequest) -> Result<ImportStart, PersistenceError> {
        ...
        let locked: bool =
            query_scalar("SELECT pg_try_advisory_xact_lock(hashtextextended($1, 0))")
                .bind(lock_key)
        ...
        .bind(&request.source.parser_name)
        .bind(&request.source.parser_version)
        .bind(i64::try_from(request.artifact.byte_length).map_err(|_| {
    ...
    async fn status(&self, filter: &StatusFilter) -> Result<Vec<RunStatus>, PersistenceError> {
        ...
        let rows = query(
            "SELECT r.id, a.id AS attempt_id, COALESCE(r.source_slug, lower(r.source_name)) source_slug,
                    ...
                    COALESCE(r.skipped_aircraft, 0) skipped_aircraft,
        ...
        let attempts = query(
            "SELECT id, ingest_run_id, attempt_number, status,
                    ...
                    skipped_aircraft, warning_count, failure_code, failure_message,
        ...
        for row in &attempts {
            ...
            let status =
                statuses.iter_mut().find(|status| status.run_id == run_id).ok_or_else(|| {
                    PersistenceError::Invariant(
                        "attempt query returned an unknown ingestion run".to_owned(),
                    )
                })?;
...
impl IngestionUnitOfWork for SqlxIngestionUnitOfWork {
    ...
    async fn mark_succeeded(&mut self, report: &ImportReport) -> Result<(), PersistenceError> {
        ...
        .bind(to_i32(report.skipped_records)?)
...
impl SqlxIngestionUnitOfWork {
    ...
    async fn promote_document(
        ...
        let source_id: i64 = query_scalar(
            "INSERT INTO aircraft_prov.sources(name,slug,source_type_code,
             ...
             VALUES($1,$2,'SCRAPED_WEB','UNVERIFIED',$3,0.20,
    ...
    async fn promote_measurements(
        ...
        for metric in &record.performance.measurements {
            ...
            self.assert_entity_value(
                document_id,
                "AIRCRAFT_VARIANT",
                variant_id,
                &format!(
                    "performance.{}",
                    metric.metric_code.as_deref().unwrap_or(&metric.source_field)
                ),
                &metric.raw_value,
                ...
                (&metric.metric_code, &metric.numeric_value, &metric.unit_code)
            {
                query(
                    "INSERT INTO aircraft_specs.weight_metrics(
                        variant_id,metric_type_code,raw_value,raw_unit_code,canonical_value,confidence)
                     VALUES($1,$2,$3::numeric,$4,
                        aircraft_ref.to_canonical($3::numeric,$4),0.20)
                     ON CONFLICT(variant_id,metric_type_code,(COALESCE(configuration,'')))
                     DO NOTHING"
                ).bind(variant_id).bind(code).bind(value).bind(unit)
                    .execute(&mut *self.transaction).await.map_err(database_error)?;
            }
            self.assert_entity_value(
                document_id,
                "AIRCRAFT_VARIANT",
    ...
    async fn promote_market(
        ...
        if record.valuation.papi_price_estimate.is_some() {
            query(
...
fn row_to_attempt_status(row: &sqlx_postgres::PgRow) -> Result<AttemptStatus, PersistenceError> {
    ...
    Ok(AttemptStatus {
        ...
        attempt_number: u32::try_from(row.get::<i32, _>("attempt_number")).map_err(|_| {
            PersistenceError::Invariant("attempt number must be positive".to_owned())
        })?,
        status: parse_status(&status)?,
        staged_records: nonnegative(row.try_get("staged_aircraft").ok()),
        promoted_records: nonnegative(row.try_get("promoted_aircraft").ok()),
...
#[allow(clippy::needless_pass_by_value)]
fn database_error(error: SqlxError) -> PersistenceError {
    let code = error
        .as_database_error()
