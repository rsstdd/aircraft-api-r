...
use aircraft_domain::ingestion::ProductionYears;
...
pub fn normalize_record(manufacturer: &str, aircraft: &str, raw: Value) -> PreparedAircraftRecord {
    ...
    let thrust =
        engine.and_then(|value| numeric(value.get("thrust"), "engine.thrust", &mut issues));
    ...
    PreparedAircraftRecord {
        ...
        propulsion: PropulsionInput {
            manufacturer: engine.and_then(|value| {
                scalar(value.get("manufacturer"), "engine.manufacturer", &mut issues)
            }),
        ...
        valuation: ValuationInput {
            ...
            for_sale_count: numeric_integer(
                object.get("for_sale_count"),
                "for_sale_count",
                &mut issues,
            ),
        },
...
mod tests {
    ...
    fn mapped_measurement_without_unit_is_flagged() {
        let record = normalize_record(
            "CESSNA",
            "172S",
            json!({"performance": {"best_cruise_speed": "124"}}),
        );
