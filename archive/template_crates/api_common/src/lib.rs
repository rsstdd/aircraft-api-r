...
use aircraft_utils::{claims::Claims, settings::structs::FederationConfig, ApiError, AircraftError};
use url::Url;

pub async fn blocking<F, T>(pool: &DbPool, f: F) -> Result<T, AircraftError>
where
  ...
  T: Send + 'static,
{
  ...
  let res = actix_web::web::block(move || {
    let conn = pool.get()?;
    let res = (f)(&conn);
    Ok(res) as Result<T, AircraftError>
  })
  .await?;
...
pub fn is_admin(local_user_view: &LocalUserView) -> Result<(), AircraftError> {
...
pub async fn get_post(post_id: PostId, pool: &DbPool) -> Result<Post, AircraftError> {
...
pub async fn get_local_user_view_from_jwt(
  ...
  pool: &DbPool,
  secret: &Secret,
) -> Result<LocalUserView, AircraftError> {
...
pub async fn get_local_user_view_from_jwt_opt(
  ...
  pool: &DbPool,
  secret: &Secret,
) -> Result<Option<LocalUserView>, AircraftError> {
  match jwt {
...
pub async fn get_local_user_settings_view_from_jwt(
  ...
  pool: &DbPool,
  secret: &Secret,
) -> Result<LocalUserSettingsView, AircraftError> {
  let claims = Claims::decode(jwt, &secret.jwt_secret)
    .map_err(|e| ApiError::err("not_logged_in", e))?
    .claims;
  let local_user_id = LocalUserId(claims.sub);
...
pub async fn get_local_user_settings_view_from_jwt_opt(
  ...
  pool: &DbPool,
  secret: &Secret,
) -> Result<Option<LocalUserSettingsView>, AircraftError> {
  match jwt {
    Some(jwt) => Ok(Some(
...
pub fn check_post_deleted_or_removed(post: &Post) -> Result<(), AircraftError> {
...
pub async fn check_downvotes_enabled(score: i16, pool: &DbPool) -> Result<(), AircraftError> {
  if score == -1 {
...
pub async fn build_federated_instances(
  ...
  hostname: &str,
) -> Result<Option<FederatedInstances>, AircraftError> {
  ...
  if federation.enabled {
    ...
    let mut linked = distinct_communities
      ...
      .collect::<Result<Vec<String>, AircraftError>>()?;
...
pub fn password_length_check(pass: &str) -> Result<(), AircraftError> {
...
pub fn site_description_length_check(description: &str) -> Result<(), AircraftError> {
...
pub fn honeypot_check(honeypot: &Option<String>) -> Result<(), AircraftError> {
  if honeypot.is_some() {
