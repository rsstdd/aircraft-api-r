...
| [`issue-implement-review`](issue-implement-review/SKILL.md) | Work exists — a tree, a branch, or a PR — and must be judged against its issue before merge | Criteria graded Satisfied / Partially satisfied / Violated / Unverified, findings by root cause, one merge verdict | No |
...
`issue-implement-review` is the one that may be pointed at work it did not plan. It re-derives the
criteria set independently — a set read off the implementation agrees with the implementation — and
it is the correctness pass `rust-review` routes to, that skill being scoped to over-engineering
alone (`rust-review/SKILL.md:92`). `rust-remediation` then closes the loop: it treats every finding
as a hypothesis to verify before editing, rejects the ones repository evidence disproves, and
re-reviews the result against every criterion rather than stopping when the findings list is
empty.
